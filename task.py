...
def help():
    sa = """Usage :-
$ ./task add "task item"    # Add a new item with priority 2 and text "hello world" to the list
...
def report():
    ...
    try:
        ...
        for i in d:
            prior = d[i]
            sys.stdout.buffer.write(f"{i}. {prior[:-1]}[{prior[-1]}] ".encode('utf8'))
            sys.stdout.buffer.write("\n".encode('utf8'))
        ...
        with open("task.txt", "r+") as f:
            ...
            for i in lines:
                if i.strip('\n') != d[no]:
                    f.write(i)
            f.truncate()
        print(f"Deleted item with index {no}")
  
    except Exception as e:
        
        print(f"Error: item with {no} does not exist. Nothing deleted.")
...
def nef():
    ...
    for i in d1:
            prior1 = d1[i]
            sys.stdout.buffer.write(f"{i}. {prior1[:-1]}[{prior1[-1]}] ".encode('utf8'))
            sys.stdout.buffer.write("\n".encode('utf8'))
